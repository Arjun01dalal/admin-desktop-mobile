...
freerasp-react-native v4 uses the `useFreeRasp(config, actions)` hook (there is no `start()`), and config fields are singular iOS `appBundleId`/`appTeamId` + Android `supportedAlternativeStores`. Native-only code must live in `*.native.ts` files with a no-op default sibling so web bundling never pulls the native module. freeRASP is also NOT in the Expo Go store client — calling it there crashes with "Exception in HostFunction"; guard with `Constants.executionEnvironment === 'storeClient'` and lazy-require it only in real builds. Phones can't reach Metro's container IP; the dev workflow must run `expo start --tunnel` (tunnel URL changes per restart; first tunnel attempt can fail transiently — retry). Root/hook/tamper threats should be sticky (never un-flagged); VPN can toggle.

## Expo tunnel flakiness
Expo's ngrok tunnel intermittently fails at startup (`CommandError: TypeError: Cannot read properties of undefined (reading 'body')` or `remote gone away`) — usually a stale exp.direct hostname session colliding after a restart. The workflow command now deletes `packages/mobile/.expo/settings.json` (forces fresh hostname randomness) and retries up to 5 times. Custom EXPO_TUNNEL_SUBDOMAIN is rejected ("remote gone away") — don't use it.

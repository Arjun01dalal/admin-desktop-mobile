...
// 1) Registry
const registry = require(path.join(ROOT, 'electron/secure/registry.cjs'));
...
const caller = fs.readFileSync(
  path.join(ROOT, 'src/screens/panel/callerResponsibility/constants.ts'),
  'utf8',
);
write(
...
let perms = fs.readFileSync(path.join(ROOT, 'src/auth/permissions.ts'), 'utf8');
perms = perms
  .replace(
...
// 4) navItems.ts
let nav = fs.readFileSync(path.join(ROOT, 'src/layout/navItems.ts'), 'utf8');
nav = nav.replace(

...
/** Laxmi HouseGames: `${formatUTCDate} - ${formatedTime}` (IST). */
...
export function HouseGamesScreen() {
  ...
  const columns = useMemo<DataTableColumn<TxnRow>[]>(
    () => [
      ...
      { key: 'userId', label: 'User ID', width: 130, render: (r) => String(r.userId || '-') },
      {
        ...
        width: 160,
        render: (r) => String(r.txnId || r.transactionId || '-'),
      },
      {
        key: 'refTxnId',
        label: 'Ref Txn ID',
        width: 160,
        render: (r) => String(r.refTxnId || '-'),
      },
      ...
      { key: 'amount', label: 'Amount', width: 110, align: 'right', render: (r) => fmt2(r.amount) },
      {
        ...
        width: 110,
        align: 'right',
        render: (r) => fmt2(r.winingPoint),
      },
      {
        key: 'roundCapacity',
        label: 'Round Capacity',
        width: 110,
        align: 'right',
        render: (r) => String(r.roundCapacity ?? '-'),
      },
  ...
  const submitWinningPoint = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Error', e instanceof Error ? e.message : 'Failed to update winning point');
    } finally {
      setWinLoading(false);
    }
      ...
      >
        ...
        >
          <Text style={styles.filterToggleText}>
            {showFilters
              ? 'Hide Search ▲'
              : `Search Filters ▼${activeFilterCount ? ` (${activeFilterCount} active)` : ''}`}
          </Text>
        </TouchableOpacity>
          ...
          <View style={styles.filterPanel}>
            ...
            <View style={styles.chipGroupRow}>
              ...
              {TYPE_OPTIONS.map((opt) => (
                ...
                >
                  <Text
                    style={[styles.chipText, draftFilters.type === opt && styles.chipTextActive]}
                  >
                    {opt || 'All'}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles.chipGroupRow}>
              ...
              {STATUS_OPTIONS.map((opt) => (
                ...
                >
                  <Text
                    style={[styles.chipText, draftFilters.status === opt && styles.chipTextActive]}
                  >
                    {opt || 'All'}
                  </Text>
                </TouchableOpacity>
          ...
          <View style={styles.totalsLine}>
            {totalCount !== null && (
              <Text style={styles.totalsText}>
                Total Count: {totalCount.toLocaleString('en-IN')}
              </Text>
            )}
            {totalAmount !== null && (
              <Text style={styles.totalsText}>
          ...
          <View style={styles.editCard}>
            ...
            <View style={styles.editActions}>
              ...
              <TouchableOpacity
                ...
                disabled={editLoading}
              >
                <Text style={styles.applyBtnText}>{editLoading ? 'Updating...' : 'Update'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
          ...
          <View style={styles.editCard}>
            ...
            <View style={styles.editActions}>
              ...
              <TouchableOpacity
                ...
                disabled={winLoading}
              >
                <Text style={styles.applyBtnText}>{winLoading ? 'Updating...' : 'Update'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>

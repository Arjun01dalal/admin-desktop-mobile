...
export function useUsersColumns(p: UseUsersColumnsParams) {
  ...
  return useMemo<CommonTableColumn<UserRow>[]>(() => {
    if (userType === 'Sub_Admin') {
      return [
        ...
        },
        {
          ...
          filter: null,
          render: (r) => {
            const current = locationDraft[r._id] ?? String(r.officeLocation || r.location || '');
          ...
          filter:
            !isCaller && canShowMobile ? (
              <FilterInput
                ...
                onSearch={search}
                placeholder={loginEmpCode ? `Emp Code (${loginEmpCode}/001)` : 'Search by Emp Code'}
              />
            ) : null,
    ...
    // Laxmi Todays_Active table — slim column set (not the full User table).
    if (userType === 'Todays_Active') {
      const cols: CommonTableColumn<UserRow>[] = [
        {
          id: 'index',
          label: '#',
      ...
      if (showMobileColumn) {
        cols.push({
          ...
          render: (r) => (
            <CallingBtn item={r} botId={botId} reasonList={reasonForUserType(userType)} hideBotCall />
          ),
        });
      }
      ...
      cols.push(
        ...
        },
        {
          ...
          render: (r) => {
            ...
            return key && appVersions[key] ? appVersions[key] : '-';
          },
        },
        {
          id: 'lastActivity',
    ...
    if (!isCaller && (userType === 'User' || userType === 'Todays_Active')) {
      cols.push({
        id: 'blockReason',
        label: (
          <>
